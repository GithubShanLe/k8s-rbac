<template>
  ...
  <div class="filter-container">
      ...
      <el-input
        v-model="queryParams.replicasetName"
        placeholder="replicaset名称"
        class="filter-item"
        style="width: 200px;"
        clearable
    ...
    <el-card shadow="hover" class="mt-20 info-card">
      <div slot="header" class="sub-header">
        <i class="el-icon-cpu"></i> replicaset列表
        <span class="sub-title-count">({{ replicasets.length || 0 }})</span>
      </div>
      <el-table 
        :data="replicasets" 
        border 
...
import { listReplicaset,getNamespaces } from '@/api/k8s'
export default {
data() {
return {
  listLoading: false,
...
toggleLabels(index) {
  // 使用 Vue.set 确保响应式更新
  if (!this.replicasets[index].labelsExpanded) {
    this.$set(this.replicasets[index], 'labelsExpanded', true);
  } else {
    this.$set(this.replicasets[index], 'labelsExpanded', false);
  }
},
...
async fetchData() {
  ...
  try {
    ...
    const { data } = await listReplicaset({
      nameSpace: this.queryParams.nameSpace,
      replicasetName: this.queryParams.replicasetName
    })
    if (data.errorCode) {
    ...
    // 初始化每个服务的标签展开状态
    this.replicasets = (data.replicasets || []).map(replicaset => ({
      ...replicaset,
