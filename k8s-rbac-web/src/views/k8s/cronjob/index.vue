  ...
  <el-card class="box-card">
    ...
    <el-card shadow="hover" class="mt-20 info-card">
      <div slot="header" class="sub-header">
        <i class="el-icon-cpu"></i> cronjob列表
        <span class="sub-title-count">({{ cronjobs.length || 0 }})</span>
      </div>
      <el-table 
        :data="cronjobs" 
        border 
...
toggleLabels(index) {
  // 使用 Vue.set 确保响应式更新
  if (!this.cronjobs[index].labelsExpanded) {
    this.$set(this.cronjobs[index], 'labelsExpanded', true);
  } else {
...
async fetchData() {
  ...
  try {
    ...
    // 初始化每个服务的标签展开状态
    this.cronjobs = (data.cronJobs || []).map(cronjob => ({
      ...cronjob,
