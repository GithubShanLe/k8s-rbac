<template>
  ...
  <div class="filter-container">
      ...
      <el-input
        v-model="queryParams.statefulsetName"
    ...
    <el-card shadow="hover" class="mt-20 info-card">
      <div slot="header" class="sub-header">
        <i class="el-icon-cpu"></i> statefulset列表
        <span class="sub-title-count">({{ statefulsets.length || 0 }})</span>
      </div>
      <el-table 
        :data="statefulsets" 
        border 
...
import { listStatefulset,getNamespaces } from '@/api/k8s'
export default {
data() {
return {
  listLoading: false,
...
toggleLabels(index) {
  // 使用 Vue.set 确保响应式更新
  if (!this.statefulsets[index].labelsExpanded) {
    this.$set(this.statefulsets[index], 'labelsExpanded', true);
  } else {
    this.$set(this.statefulsets[index], 'labelsExpanded', false);
  }
},
...
async fetchData() {
  ...
  try {
    ...
    const { data } = await listStatefulset({
      nameSpace: this.queryParams.nameSpace,
      statefulsetName: this.queryParams.statefulsetName
    })
    if (data.errorCode) {
    ...
    // 初始化每个服务的标签展开状态
    this.statefulsets = (data.statefulsets || []).map(statefulset => ({
      ...statefulset,
