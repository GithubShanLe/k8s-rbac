  ...
  <el-card class="box-card">
    ...
    <el-card shadow="hover" class="mt-20 info-card">
      <div slot="header" class="sub-header">
        <i class="el-icon-cpu"></i> job列表
        <span class="sub-title-count">({{ jobs.length || 0 }})</span>
      </div>
      <el-table 
        :data="jobs" 
        border 
...
import { listJob ,getNamespaces} from '@/api/k8s'
export default {
data() {
return {
  listLoading: false,
...
toggleLabels(index) {
  // 使用 Vue.set 确保响应式更新
  if (!this.jobs[index].labelsExpanded) {
    this.$set(this.jobs[index], 'labelsExpanded', true);
  } else {
    this.$set(this.jobs[index], 'labelsExpanded', false);
  }
},
